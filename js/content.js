class ElementBlockerContent {
  ...
  getCurrentDomain() {
    return getDomainFromUrl(window.location.href);
  }
  ...
  updateBlockingStyles() {
    ...
    if (this.isEnabled && this.blockedClasses.length > 0) {
      ...
      const activeClasses = getActiveBlockedClasses(this.blockedClasses, this.currentDomain);
  ...
  async addBlockedClass(className) {
    try {
      ...
      // 向后兼容：转换旧格式数据
      blockedClasses = normalizeBlockedClasses(blockedClasses);

      // 检查是否已存在（同名同域名）
      if (!isDuplicateClass(blockedClasses, className, this.currentDomain)) {
        // 新增的屏蔽项带上当前域名
        blockedClasses.push({
          className,
          enabled: true,
