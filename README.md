...
- **导出配置**：在设置页面点击"导出配置"，将所有规则保存为 JSON 文件
- **导入配置**：在设置页面点击"导入配置"，可选择覆盖或合并现有配置
...
| 函数                        | 描述                               |
| --------------------------- | ---------------------------------- |
| `normalizeBlockedClasses()` | 向后兼容：转换旧格式数据结构       |
| `getDomainFromUrl()`        | 从 URL 提取域名                    |
| `generateSelector()`        | 生成 CSS 选择器（支持单/组合类名） |
| `getActiveBlockedClasses()` | 获取当前域名下激活的屏蔽项         |
| `groupByDomain()`           | 按域名分组屏蔽项                   |
| `isDuplicateClass()`        | 检查屏蔽项是否重复                 |
| `showTempMessage()`         | 显示临时消息提示                   |
